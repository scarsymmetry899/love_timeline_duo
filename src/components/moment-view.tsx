  ...
  return (
    <main className="mx-auto max-w-xl overflow-x-clip px-5 pb-24 pt-[max(1.25rem,env(safe-area-inset-top))]">
          ...
          {moment.photos.map((p, i) => (
            <figure key={p.id} className={`polaroid relative flex-none ${TILTS[i % TILTS.length]}`}>
              ...
              <img
                ...
                className="max-h-[62vh] w-auto max-w-[78vw] bg-sunken object-contain sm:max-w-md"
              />
              <figcaption className={`${pen} h-10 pt-1 text-center text-xl text-ink-muted`}>
      ...
      <article className="relative mt-8">
        ...
        <p className="type pr-24 text-xs uppercase text-ink-muted">{fmtLong(moment.moment_date)}</p>
        ...
        {moment.place && (
          <p className="luggage-tag mt-5">
            <span className="type text-[0.6rem] uppercase text-ink-muted">where</span>
          ...
          <ul className="mt-7 flex flex-col gap-4">
            {moment.links.map((l) =>
              l.provider === "spotify" || l.provider === "youtube" ? (
                <li key={l.id}>
                  <a href={l.url} target="_blank" rel="noreferrer" className="cassette">
                    <span className="cassette-label">
                      <span className="type text-[0.6rem] uppercase text-ink-muted">side a · our song</span>
                ...
                <li key={l.id}>
                  <a href={l.url} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl bg-surface px-4 py-3 font-semibold shadow-sm">
                    <span className="grid h-9 w-9 flex-none place-items-center rounded-full bg-sunken"><LinkIcon /></span>
                    <span className="min-w-0">
                      <span className="block">{l.title || linkLabel(l.provider)}</span>
                      <span className="block truncate text-sm font-normal text-ink-muted">{new URL(l.url).hostname.replace(/^www\./, "")}</span>
                    </span>
                  </a>
                </li>
              ),
