  ...
  return (
    <form onSubmit={save} className="flex flex-col gap-8 pb-4">
      ...
      <div className="text-center">
        ...
        <h1 className="text-[2.1rem] font-extrabold leading-tight tracking-tight">What do you want to keep?</h1>
      </div>
          ...
          <figure className={`polaroid relative ${photos.length > 1 ? "stacked" : ""}`}>
            ...
            {cover ? (
              <button type="button" onClick={pickPhotos} disabled={busy || photos.length >= MAX_PHOTOS} className="block w-full" aria-label="Add more photos">
                ...
                <img src={cover} alt="" className="aspect-[4/5] w-full bg-paper-deep object-cover" onError={(e) => (e.currentTarget.style.visibility = "hidden")} />
              </button>
            ) : (
              <button
                type="button"
                onClick={pickPhotos}
                disabled={busy}
                className="empty-photo flex aspect-[4/5] w-full flex-col items-center justify-center gap-2 text-ink-soft"
              >
            ...
            <figcaption className="px-1 pb-3 pt-2 text-center">
              ...
              <span className="type mt-1 block text-[0.65rem] uppercase text-ink-soft">{prettyDate}</span>
            </figcaption>
          </figure>
        </div>
          ...
          <div className="mt-6 flex max-w-full gap-3 overflow-x-auto px-2 pb-2 pt-2">
            {photos.map((p, i) => (
              <div key={p.url} className="relative flex-none">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.url} alt={`Photo ${i + 1}`} className="h-16 w-16 rounded-lg bg-paper-deep object-cover shadow" />
                <button
                  type="button"
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="heading" className="type text-xs uppercase text-ink-soft">title</label>
        <input
          id="heading"
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="date" className="type text-xs uppercase text-ink-soft">the day it happened</label>
        <input id="date" type="date" required max={today} className="field" value={date} onChange={(e) => setDate(e.target.value)} />
      </div>

      <div className="flex flex-col gap-2">
        <label htmlFor="body" className="type text-xs uppercase text-ink-soft">the story</label>
        <textarea
          id="body"
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="flex flex-col gap-2">
          <label htmlFor="place" className="type inline-flex items-center gap-1.5 text-xs uppercase text-ink-soft"><PinIcon className="h-3.5 w-3.5" /> where</label>
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="link" className="type inline-flex items-center gap-1.5 text-xs uppercase text-ink-soft"><MusicIcon className="h-3.5 w-3.5" /> your song</label>
      ...
      {error && (
        <div role="alert" className="rounded-2xl border-2 border-[var(--danger)] bg-polaroid p-4 text-[var(--danger)]">
          <p>{error}</p>
