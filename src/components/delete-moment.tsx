...
export default function DeleteMoment({ id }: { id: string }) {
  ...
  if (!confirming) {
    return (
      <button type="button" className="inline-flex items-center gap-1.5 text-sm font-semibold text-ink-muted" onClick={() => setConfirming(true)}>
        <TrashIcon /> Delete this memory
      </button>
    );
  }
  return (
    <div className="rounded-2xl bg-surface p-4" role="group" aria-label="Confirm delete">
